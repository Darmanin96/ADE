...
public class tutorEmpresaSelectedController implements Initializable {
    ...
    private boolean validarCampos(){
        ...
        boolean ContactoValido = (
                !Contacto.getText().isBlank() && Contacto.getText().matches("^\\+?[0-9]{1,4}?[\\s.-]?\\(?[0-9]{1,4}\\)?[\\s.-]?[0-9]{1,4}[\\s.-]?[0-9]{1,9}$")) &&
                (!Contacto.getText().isBlank() && Contacto.getText().matches("^[\\w.%+-]+@[\\w.-]+\\.[a-zA-Z]{2,}$"));
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        validarCampo(Contacto, "^\\+?[0-9]{1,4}?[\\s.-]?\\(?[0-9]{1,4}\\)?[\\s.-]?[0-9]{1,4}[\\s.-]?[0-9]{1,9}$", "Introduzca el contacto correctamente.");
        validarCampo(Contacto, "^[\\w.%+-]+@[\\w.-]+\\.[a-zA-Z]{2,}$", "Introduzca el contacto correctamente.");
    }
