...
public class AlumnSelectedController implements Initializable {
    ...
    private void actualizar() {
        ...
        String id = alumnController.getId_Alumn().getText().toString();
        Integer idDeVerdad = Integer.parseInt(id);
        String sql = "UPDATE alumnos SET Id_Alumno=?, nombre = ?, apellido = ?, tutor_empresa = ?, tutor_grupo = ?, pendientes = ?, contacto = ?, programa = ? WHERE id = ?";
        try (Connection con = Conectar.getConnection();
             PreparedStatement stmt = con.prepareStatement(sql)) {
            stmt.setInt(0,idDeVerdad);
